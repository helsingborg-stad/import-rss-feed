...
namespace ImportRssFeed\Helper;
...
class CacheBust
{
    ...
    {
        $jsonPath = IMPORTRSSFEED_PATH . apply_filters('ImportRssFeed/Helper/CacheBust/RevManifestPath', 'dist/rev-manifest.json');

        if (file_exists($jsonPath)) {
            return json_decode(file_get_contents($jsonPath), true);
        } elseif (WP_DEBUG) {
            echo '<div style="color:red">Error: Assets not built. Go to ' . IMPORTRSSFEED_PATH . ' and run gulp. See '. IMPORTRSSFEED_PATH . 'README.md for more info.</div>';
        }
    }
}
